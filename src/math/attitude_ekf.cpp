...
bool attitude_ekf(EKFState &state,
    ...
    MatrixF<6, 6> A = ZeroMatrix<float, 6, 6>();
    A.slice<3, 3>(0, 0) = cross_mat(y_g);
    A.slice<3, 3>(0, 3) = -0.5f*IdentityMatrix<float, 3>();

    MatrixF<6, 6> Q = ZeroMatrix<float, 6, 6>();
    Q.slice<3, 3>(0, 0) = 0.25f*diag(ConstMatrix<float, 3, 1>(R_g));
    Q.slice<3, 3>(3, 3) = diag(ConstMatrix<float, 3, 1>(Q_b_g));

    MatrixF<6, 6> A_d = IdentityMatrix<float, 6>() + dt*A;
    MatrixF<6, 6> Q_d = dt*Q + (0.5f*dt*dt)*(A*Q + Q*tr(A));

    state.x = VectorF<6>(A_d*state.x);
    state.P = MatrixF<6, 6>(A_d*state.P*tr(A_d) + Q_d);
    ...
    MatrixF<6, 6> H = ZeroMatrix<float, 6, 6>();
    if (acc_en) {
    ...
    state.x = VectorF<6>(state.x + tr(Kt)*(z - H*state.x));
    state.P = MatrixF<6, 6>((IdentityMatrix<float, 6>() - tr(Kt)*H)*state.P);
    for (int r=0; r<9; r++)
        for (int c=0; c<r; c++)
            state.P(c, r) = state.P(r, c);
